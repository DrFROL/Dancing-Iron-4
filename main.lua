...
function AimUpdate(self, dt)
    ...
    if Collides(self,castle) then
        self.speed = 0
        if self.atkTimer >= self.atkCd then
            self.atkTimer = 0
            castle.xp = castle.xp - mob.damage
            if castle.xp == 0 then
                mob.damage = 0
            end
        end
...
function Tower_checker()
    if Tower_check then
        if timer_tower < 3 then
            timer_tower = timer_tower + 1
        else
            timer_tower = 0 
        end
        if love.mouse.isDown(1) and timer_tower == 3 then
...
function Dot_checker()
    if dot_check then
        ...
        if love.mouse.isDown(1) and timer_dot == 3 then
            ms_x, ms_y = love.mouse.getPosition()
...
function Miner_checker()
    if Miner_check then
        if timer_miner < 3 then
            timer_miner = timer_miner + 1
        else
            timer_miner = 0 
        end
        if love.mouse.isDown(1) and timer_miner == 3 then
            ms_x, ms_y = love.mouse.getPosition()
...
function Rock_Axer_checker()
    if rock_axer_check then
        if timer_rock_axer < 3 then
            timer_rock_axer = timer_rock_axer + 1
        else
            timer_rock_axer = 0 
        end
        if love.mouse.isDown(1) and timer_rock_axer == 3 then
            ms_x, ms_y = love.mouse.getPosition()
...
function Tree_checker()
    if tree_check then
        if timer_tree < 3 then
            timer_tree = timer_tree + 1
        else
            timer_tree = 0 
        end
        if love.mouse.isDown(1) and timer_tree == 3 then
            ms_x, ms_y = love.mouse.getPosition()
...
function Resurs()
    chet = chet + 1
    if chet == 120 then 
        rock = rock + #rock_axers * 10
        treess = treess + #trees * 10
        steel = steel + #miners * 10
        gold = gold + 5
...
function Destroyer(mobs, dots)
    ...
    if chet_destroy == cld_dot - #dots * 7 then
...
function love.draw()
    ...
    love.graphics.print("Until the wave: "..timer.." ")
    love.graphics.print(castle.xp.."hp",castle.x+15,castle.y-30)
